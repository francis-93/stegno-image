...
def encode():
    if request.method == 'POST':
        ...
        # Include the encrypted message to be passed to the template
        encrypted_message = base64.b64encode(message.encode()).decode()  # Just encoding message in base64 for displaying

        return render_template('encode_result.html', chacha_key=chacha_key, encoded_image=output_image, encrypted_message=encrypted_message)
...
def decode():
    if request.method == 'POST':
        ...
        # Extract image without secret message
        img = Image.open(stego_image)
        width, height = img.size
        pixels = img.load()

        # Create a new image by resetting the least significant bit of the red channel
        extracted_image = img.copy()
        for y in range(height):
            for x in range(width):
                r, g, b = pixels[x, y]
                r = r & ~1  # Reset the least significant bit
                extracted_image.putpixel((x, y), (r, g, b))

        # Save the extracted image
        extracted_image_path = 'static/extracted_image.png'
        extracted_image.save(extracted_image_path)

        # Return the decoded message and the extracted image as a downloadable link
        return render_template('decode_result.html', decoded_message=decoded_message, extracted_image=extracted_image_path)
